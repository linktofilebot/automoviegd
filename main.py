...
# --- এনভায়রনমেন্ট ভেরিয়েবলস (Render থেকে আসবে) ---
API_ID = int(os.getenv("API_ID", "12345"))
API_HASH = os.getenv("API_HASH", "your_api_hash")
BOT_TOKEN = os.getenv("BOT_TOKEN", "your_bot_token")
MONGO_URL = os.getenv("MONGO_URL", "your_mongodb_url")
CHANNEL_ID = int(os.getenv("CHANNEL_ID", "-100123456789"))
ADMIN_PASS = os.getenv("ADMIN_PASS", "admin123") # এটাই আপনার লগইন পাসওয়ার্ড

# --- ডাটাবেস ও বট ---
app = FastAPI()
db_client = AsyncIOMotorClient(MONGO_URL)
db = db_client['cinema_final_master']
...
# --- সেটিংস হেল্পার (যাতে Internal Server Error না আসে) ---
async def get_site_conf():
    try:
        ...
        return {"site_name": "Setup Pending", "notice": "DB Not Connected", "total_steps": 0}

# --- প্রফেশনাল লাইটিং CSS ---
CSS = """
<style>
    ...
    .header { background: #10141d; padding: 20px; text-align: center; border-bottom: 2px solid var(--primary); box-shadow: 0 0 20px var(--primary); }
    ...
    .btn { background: linear-gradient(45deg, var(--primary), var(--secondary)); color: white; padding: 10px 20px; border-radius: 5px; text-decoration: none; font-weight: bold; border: none; cursor: pointer; transition: 0.3s; }
    ...
    .card { background: var(--card); border-radius: 10px; overflow: hidden; border: 1px solid #222; text-align: center; transition: 0.3s; }
    ...
    .badge { background: rgba(0,242,255,0.1); color: var(--primary); padding: 5px 10px; border-radius: 4px; border: 1px solid var(--primary); font-size: 12px; margin-right: 5px; }
...
async def home(q: str = Query(None), cat: str = Query(None)):
    ...
    try:
        ...
        return "<h1>MongoDB Connection Error!</h1><p>Check IP Whitelist (0.0.0.0/0)</p>"
    ...
    for c in cats: html += f"<a href='/?cat={c['name']}' class='badge'>{c['name']}</a>"
...
async def movie_view(id: str):
    ...
    html += f"<p><b>Cast:</b> {item.get('actors')}</p><p><b>Story:</b> {item.get('description')}</p><h3>Download Links:</h3>"
    if item['type'] == 'movie':
...
async def verify_page(id: str, ctype: str, cidx: str, step: int):
    ...
    return f"<html><head>{CSS}</head><body style='text-align:center;padding-top:100px;'><div class='container'><div class='admin-box' style='display:inline-block'><h2>Step {step+1}/{total}</h2><a href='{curr_ad}' target='_blank' onclick=\"window.location.href='/verify/{id}/{ctype}/{cidx}/{step+1}'\" class='btn'>Click to Continue</a></div></div></body></html>"

# --- অ্যাডমিন লগইন ও ড্যাশবোর্ড ---

@app.get("/admin", response_class=HTMLResponse)
async def admin_page(request: Request):
    # কুকি থেকে পাসওয়ার্ড চেক
    ...
    cats = await category_col.find().to_list(100)
    contents = await content_col.find().sort("_id", -1).to_list(10)
    
    html = f"<html><head><title>Admin</title>{CSS}</head><body><div class='container'><h1 class='neon-text'>🛡️ Admin Dashboard</h1>"
    # ১. সেটিংস
    html += f"<div class='admin-box'><h3>⚙️ ১. Global Settings</h3><form action='/admin/save_settings' method='post'>Site Name: <input name='site_name' value='{conf['site_name']}'>Notice: <textarea name='notice'>{conf['notice']}</textarea>Ad Steps: <input type='number' name='total_steps' value='{conf['total_steps']}'>Ad URLs: <textarea name='ad_urls' rows='3'>{conf['ad_step_urls']}</textarea><button class='btn'>Save Settings</button></form></div>"
    # ২. কন্টেন্ট
    html += f"<div class='admin-box'><h3>🎬 ২. Content Management</h3><form action='/admin/add_content' method='post'><input name='title' placeholder='Title' required><input name='poster' placeholder='Poster URL'><input name='rating' placeholder='Rating'><input name='lang' placeholder='Lang'><input name='actors' placeholder='Actors'><input name='trailer' placeholder='Trailer URL'><input name='quality' placeholder='Quality'><textarea name='desc' placeholder='Description'></textarea><select name='type'><option value='movie'>Movie</option><option value='series'>Series</option></select><select name='cat'>{''.join([f'<option value=\"{c[\"name\"]}\">{c[\"name\"]}</option>' for c in cats])}</select>JSON Data: <textarea name='json' rows='3'></textarea><button class='btn'>Publish Content</button></form></div>"
    # ৩. ট্যাক্সোনোমি
    html += """<div class='admin-box'><h3>📂 ৩. OTT & Categories</h3><form action='/admin/add_cat' method='post'><input name='name' placeholder='Add Category'><button class='btn'>Add</button></form></div>"""
    # লিস্ট
